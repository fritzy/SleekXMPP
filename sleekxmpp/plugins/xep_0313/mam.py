...
class Archive_Query:
    def __init__(self, xmpp, start=None, end=None, with_jid=None, continue_after=None,
                 timeout=None, callback=None, number_of_queried_elements=None, collect_all=True):
        self._xmpp = xmpp
        ...
        if ( number_of_queried_elements ):
            iq['mam']['rsm']['max'] = str(number_of_queried_elements)
        if ( continue_after ):
    ...
    def __send_async_query(self):
        self._iq.send(block=False, timeout=self.timeout, callback=self.__handle_async_result)
...
class XEP_0313(BasePlugin):
    ...
    dependencies = set(['xep_0004', 'xep_0030', 'xep_0050', 'xep_0059', 'xep_0297'])
    stanza = stanza
    ...
    def plugin_init(self):
        ...
        register_stanza_plugin(stanza.MAM_Fin, self.xmpp['xep_0059'].stanza.Set)
        ...
        self._open_queries = dict()


    def retrieve(self, start=None, end=None, with_jid=None, continue_after=None,
                 timeout=None, callback=None, number_of_queried_elements=None, collect_all=True):


        query = Archive_Query(self.xmpp, start, end, with_jid, continue_after, timeout, callback, number_of_queried_elements, collect_all)
